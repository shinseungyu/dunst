document.addEventListener(`DOMContentLoaded`, function () { 
    ...
    menuBtn.addEventListener(`click`, function () { 
        ...
        // 토글 기능 두가지를 진행할 경우 브라우저 오류 발생할 수 있음 contains() 메서드를 사용하면 좋음
        const menuHas = menuBtn.classList.contains(`active`);
        // menuBtn에 active 클래스명이 있다
        //contains() 메서드는 특정 문자열이 포함되어있는지 여부를 확인하고 포함되어있으면 true를 반환, 그렇지 않으면 false 를 반환
    ...
    // scroll
    // 200px 이상에서 .header_area,.header_logo,.menu_btn on 추가 / 그 밖에 경우  .header_area,.header_logo,.menu_btn on제거 (.header-area,.header-logo,.menu-btn 한번에 선택해서 진행해보기)
